...
sp_api_params_server = {
    ...
    "attributes" : environ.get('SINGPASS_ATTRIBUTES'),
...
appointment_booking_api = environ.get('appointment_booking_service_URL')+'/book_appointment'
...
@app.route('/')
def index():
    state = "page_add__guid_"
    try:
        doctors = retrieveDoctors()
        code = request.args.get('code')
        if code is None:
            return render_template('add_appointment.html',api=sp_api_params_client,doctors=doctors,state=state)
        else:
            res = retrieve_myinfo(code)
            return render_template('add_appointment.html',api=sp_api_params_client,prefill=res,doctors=doctors,state=state)
    except MyInfoError as me:
        return render_template('add_appointment.html',api=sp_api_params_client,error="Access Token expired!",state=state)
    except Exception as e:
        return render_template('add_appointment.html',api=sp_api_params_client,error=e,state=state)


@app.route('/doctor_timeslots/<int:id>',methods=['GET'])
def doctor_timeslots(id):
    try:
...
def book_appointment():
    try:
        ...
        # print(appointment)
        res = invoke_http(url=appointment_booking_api,method='POST',json=appointment)
        return res,res['code']
    except Exception as e:
        return {"message":"An error occurred","data":e},500
...
def retrieve_myinfo(code):
    ...
    try:
        ...
        prefill = {
            "nric" : person_res["uinfin"]['value'],
            "name" : person_res["name"]['value'],
            "sex" : person_res["sex"]['code'],
        }
        return prefill
